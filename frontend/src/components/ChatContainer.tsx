...
function WelcomeScreen() {
  const features = [
    {
      title: '矛盾分析',
      desc: '帮你抓住主要矛盾和矛盾的主要方面',
    },
    {
      title: '阶段判断',
      desc: '分析当前形势，判断所处发展阶段',
    },
    {
      title: '调查研究',
      desc: '提供调查研究方法，了解真实情况',
    },
    {
      title: '策略制定',
      desc: '制定斗争策略，团结朋友、孤立敌人',
    },
  ];
    ...
    <div className="flex-1 flex flex-col items-center justify-center px-4 py-8 overflow-y-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 w-full max-w-lg">
        {features.map((feature, i) => (
          <div
            key={i}
            className="p-4 rounded-xl bg-surface border border-border shadow-soft hover:shadow-medium hover:border-primary/30 transition-all cursor-default group"
          >
      ...
      <p className="text-xs text-text-muted mt-6 text-center">
        在下方输入框中描述你面临的问题，我会用教员的思想方法帮你分析
      </p>
    </div>
  );
}
...
export function ChatContainer({ messages, isThinking }: ChatContainerProps) {
  ...
  return (
    <div
      ...
      className="flex-1 overflow-y-auto px-4 md:px-6 py-4 md:py-6 scroll-smooth"
    >
